...
export const useBrandMessages = (user, activeTab, initialConvId, showToast) => {
  ...
  const loadConversations = useCallback(async () => {
    try {
      const data = await brandService.getConversations();
  ...
  const loadMessages = useCallback(async (convId) => {
    if (!convId) return;
    try {
      const data = await brandService.getMessages(convId);
  ...
  const handleSendMessage = async (e, forcedText) => {
    ...
    try {
      const newMsg = await brandService.sendMessage(selectedConvId, textToSend);
...
export default useBrandMessages;
