...
export default function CampaignCard({ campaign, applied, onApply, saved, onSave, onClick, currentUserId, onViewBrand }) {
  ...
  return (
    <div className="post-card glass-panel" style={{ marginBottom: '1.5rem', position: 'relative', cursor: 'pointer', padding: '1.25rem' }} onClick={onClick}>
      <div className="post-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
        <div 
        ...
        >
          <div className="post-author-avatar" style={{ background: 'var(--accent-gradient)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
            {campaign.brandName?.[0] || 'B'}
          </div>
      ...
      <div className="social-engagement" style={{ marginTop: '0.5rem', borderTop: '1px solid var(--border-light)', paddingTop: '0.5rem' }}>
        {/* Counts Row */}
      ...
      {showComments && (
        <div className="post-comments-section" onClick={(e) => e.stopPropagation()} style={{ marginTop: '1rem', borderTop: '1px solid var(--border-light)', paddingTop: '1rem' }}>
            ...
            {comments.map((comment, i) => (
              <div key={i} className="comment-item" style={{ display: 'flex', gap: '0.75rem' }}>
                <div className="comment-avatar" style={{ width: '32px', height: '32px', borderRadius: '50%', background: 'var(--surface-alt)', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
                  {comment.user?.avatar
                    ? <img src={comment.user.avatar.startsWith('http') ? comment.user.avatar : `http://localhost:5000${comment.user.avatar}`} alt="Avatar" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : <span style={{ fontSize: '0.8rem', fontWeight: 'bold' }}>{comment.user?.name?.[0]}</span>}
                </div>
                <div className="comment-bubble" style={{ background: 'rgba(255, 255, 255, 0.05)', backdropFilter: 'blur(8px)', padding: '0.75rem 1rem', borderRadius: '0 16px 16px 16px', fontSize: '0.9rem', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
          ...
          <form className="comment-form" onSubmit={handleComment} style={{ display: 'flex', gap: '0.5rem' }}>
            <input
              type="text"
            ...
            />
            <button type="submit" disabled={!newComment.trim() || isSubmittingComment} style={{ background: 'var(--primary)', color: 'white', border: 'none', padding: '0 1rem', borderRadius: '20px', cursor: 'pointer', opacity: (!newComment.trim() || isSubmittingComment) ? 0.5 : 1 }}>
              <Send size={16} />
            </button>
          </form>
        </div>
