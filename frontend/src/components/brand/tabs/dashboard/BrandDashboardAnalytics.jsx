...
export default function BrandDashboardAnalytics() {
  ...
  return (
    <Box className="dashboard-analytics-container brand-dashboard-analytics-container analytics-page" sx={{ animation: 'fadeIn 0.3s ease', pb: 6 }}>

      {/* ── Header ──────────────────────────────────────────────────────── */}
      <Box className="analytics-page-header" sx={{ mb: 2 }}>
        <Typography variant="h5" sx={{ fontWeight: 800, color: 'var(--text-primary, #1c1e21)', mb: 0.25 }}>
          Campaign Analytics
        </Typography>
      ...
      <Box className="analytics-kpi-grid" sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.5, mb: 3 }}>
        {METRICS.map(metric => {
          ...
          const value    = toSafeNumber(analytics?.totals?.[metric.id]);
          const trend    = toSafeNumber(analytics?.trends?.[metric.id]);
          ...
          return (
            ...
            >
              <CardContent className="analytics-kpi-card-content" sx={{ p: '1rem !important' }}>
                <Box className="analytics-kpi-label-row" sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1, color: isSelected ? metric.color : 'var(--text-secondary, #65676b)' }}>
                ...
                {loading ? (
                  ...
                  <Typography className="analytics-kpi-value" variant="h6" sx={{ fontWeight: 700, fontSize: '1.35rem', color: 'var(--text-primary, #1c1e21)', lineHeight: 1 }}>
                    {metric.isCurrency && '₹'}
                    {formatCount(value)}
                  </Typography>
                )}
        ...
        <>
          <Box className="analytics-inline-stat" sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1.5 }}>
            ...
            <Typography variant="body1" sx={{ fontWeight: 700, color: 'var(--text-primary)' }}>
              {formatCount(analytics.totals.reach)}
            </Typography>
          </Box>
      ...
      {selectedMetric === 'engagement' && !loading && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, width: '100%' }}>
          ...
          <Paper className="glass-panel analytics-panel analytics-breakdown-panel" elevation={0} sx={{ p: 4, width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          ...
          <Paper className="glass-panel analytics-panel analytics-breakdown-panel" elevation={0} sx={{ p: 4, width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {(analytics?.engagementBreakdown?.engagement || [
                ...
                const itemValue = toSafeNumber(item.value);
                const totalEng = toSafeNumber(analytics?.engagementBreakdown?.totals?.engagement);
                const pct = calculatePercentage(itemValue, totalEng);
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, width: '100%' }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
            ...
            <Typography variant="h5" sx={{ fontWeight: 800, color: 'var(--primary, #1877f2)' }}>
              {formatCount(analytics?.totals?.followers)}
            </Typography>
          </Box>
          ...
          <Paper className="glass-panel analytics-panel analytics-breakdown-panel" elevation={0} sx={{ p: 4, width: '100%' }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              {analytics.breakdown.byCategory.map((item, i) => {
                const itemValue = toSafeNumber(item.value);
                const maxVal = toSafeNumber(analytics.breakdown.byCategory[0]?.value);
                const pct = getCategoryRelativePct(itemValue, maxVal);
      ...
      {!loading && analytics?.insights && (
        <Box className="glass-indicator analytics-insight-panel" sx={{ 
          mt: 3, p: 2.5, 
